...
import { PRESALE_PAUSED } from '@/src/constants/presale';
...
const SpinButton = ({ onSpin, isDisabled, isSpinning }: { onSpin: () => void; isDisabled: boolean; isSpinning: boolean }) => {
  const shouldReduceMotion = useReducedMotion();
  ...
  return (
    <motion.button
      ...
      tabIndex={0}
    >
      {/* Button Highlight - Embossed Effect */}
      ...
      >
        ...
        <div className="bg-gray-900 rounded-lg p-4 mb-6 border border-gray-700">
          <div className="flex items-center justify-between">
            <code className="text-yellow-400 text-sm font-mono break-all mr-2">
              {contractAddress}
            </code>
            <button
              onClick={handleCopy}
        ...
        <div className="space-y-3">
          ...
          {/* Pre-Sale CTA (only after jackpot unlocked) */}
          {jackpotUnlocked && (
            <button
              onClick={onOpenPreSale}
              className={`w-full font-bold py-3 px-6 rounded-xl transition-colors focus:outline-none focus:ring-2 ${
                PRESALE_PAUSED
                  ? 'bg-gray-600 text-gray-300 cursor-not-allowed focus:ring-gray-400'
                  : 'bg-green-600 hover:bg-green-500 text-white focus:ring-green-400'
              }`}
              disabled={PRESALE_PAUSED}
            >
              Join Pre-Sale
            </button>
          )}
...
const PreSaleCTA = ({ onOpenPreSale, jackpotUnlocked }: { onOpenPreSale: () => void; jackpotUnlocked: boolean }) => {
  ...
  return (
    ...
    >
      <button
        onClick={onOpenPreSale}
        className={`px-8 py-4 font-bold text-lg rounded-full transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-offset-2 focus:ring-offset-gray-900 ${
          PRESALE_PAUSED
            ? 'bg-gray-600 text-gray-300 cursor-not-allowed focus:ring-gray-400'
            : 'bg-gradient-to-r from-green-600 to-green-700 hover:from-green-500 hover:to-green-600 text-white shadow-lg hover:shadow-xl hover:shadow-green-500/30 focus:ring-green-400'
        }`}
        disabled={PRESALE_PAUSED}
      >
        Join Pre-Sale
      ...
      {PRESALE_PAUSED && (
        <p className="text-gray-500 text-sm mt-2">Pre-sale is currently paused</p>
      )}
    </motion.div>
  );
};

const JackpotModal_OLD = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  const [copied, setCopied] = useState(false);
  const contractAddress = 'ALLLzDbTFq8VXYwG4awsXGtv7DKU4YNHUqkJmDS39zMa';
...
export const SlotMachine = ({ seed, durationsMs, gapMs, className }: SlotMachineProps) => {
  ...
  return (
    <>
      ...
      <PreSaleCTA 
        onOpenPreSale={() => setShowPreSaleModal(true)}
        jackpotUnlocked={state.jackpotUnlocked}
      />
    </>
  );
